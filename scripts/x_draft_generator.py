...
【記事の選び方（重要）】
- 「昨日までに発表されていない新しい情報」かつ「ビジネスインパクトが最も大きい」記事を1つ選ぶ
- 以下の記事は選ばない:
  - 規制・政策の詳細がまだ確定していない・報道が曖昧な記事
  - 日本のビジネス現場への影響経路が論理的に説明できない記事
  - 「〜が検討中」「〜の可能性」など推測段階の記事
- 選んだ記事と因果関係が明確な関連ニュースがあれば補足として絡める。その際、情報源が異なる内容は必ず「○○が〜したことで」「また、△△では〜」のように出典を明示して切り分けること

【投稿の論理構造（飛躍なく順序立てて）】
以下の4ステップを口語でつなぐ。各ステップは必ず前のステップから論理的に導けること:

1. 事象: 何がどうなったか（記事の事実のみ、憶測は含めない。複数記事を使う場合は出典ごとに切り分けて言及する）
2. 領域: この事象が影響を与えるのはどの業界・機能・ビジネスモデルか（「テクノロジー企業」のような括りは不可）
3. なぜ脅威/機会になるか: 2の領域において、なぜ今の前提や競争構造が変わるのか（「〜の可能性がある」で終わらず、その根拠を書く）
4. 企業として何を検討すべきか: 3から論理的に導かれる具体的な検討事項（「AIを活用すべき」「戦略を見直す」のような抽象アクションは不可）

【文体のイメージ】
「GoogleがXXを始めたことで、広告業界では△△が起きうる。なぜなら□□だから。これを踏まえると◇◇の単価設計を見直す必要があるんじゃないかな」
「○○ってつまり△△ということで、日本の✕✕業界だと□□が直撃する。今まで◇◇で守れていた部分が通用しなくなるから、△△という論点を経営議題に上げるべきじゃないかな」

【自己レビュー（出力前に必ず確認・書き直し）】
投稿文を一度書いたら、出力する前に以下を全項目チェックし、1つでも×なら書き直すこと:

□ 記事選定: 除外ルール（詳細未確定の規制・推測段階の情報）に引っかかる記事を使っていないか
□ ソース帰属: 複数記事を使う場合、各情報がどの記事に基づくか文中で分かるか（情報の混在がないか）
□ ①事象: 記事の事実のみを述べているか（「可能性がある」「〜か」等の憶測になっていないか）
□ ②領域: 具体的な業界・機能・ビジネスモデルで特定されているか
□ ③なぜ: 「なぜその領域に影響が出るのか」の因果が文中に明示されているか
□ ④how: 企業担当者が「具体的に何をすればよいか（何の・どこを・どう検討するか）」が特定できるか
□ 論理飛躍なし: ①→②→③→④が順番通りに導かれているか

【出力形式】
以下のJSONのみ出力すること（前置き・説明・マークダウン不要）:
{
  "draft": "投稿文（140文字以内、末尾にハッシュタグ2〜3個）",
  "referenced_urls": ["実際に投稿文で言及した記事のURLのみ列挙"]
}
...
def generate_x_draft(content: str) -> tuple[str, list[str]]:
    """投稿文と実際に参照したURLリストを返す"""
    ...
    for attempt in range(3):
        try:
            ...
            referenced_urls = parsed.get("referenced_urls", [])
            log.info(f"生成完了 — {len(draft)} 文字 / 参照URL: {len(referenced_urls)} 件")
            return draft, referenced_urls
...
def main() -> None:
    ...
    post, referenced_urls = generate_x_draft(content)
    log.info(f"投稿文:\n{post}")

    # 投稿文で実際に言及したURLのみ参考資料として表示
    used_urls = [u for u in urls if u["url"] in referenced_urls]
